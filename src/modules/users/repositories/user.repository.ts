...
export const setPreference = async (userId: number, foodCategoryId: number): Promise<void> => {
  ...
  try {
    await pool.query(
      `INSERT INTO user_prefer_category (category_id, user_id) VALUES (?, ?);`,
...
export const getUserPreferencesByUserId = async (userId: number): Promise<any[]> => {
  ...
  try {
    const [preferences] = await pool.query<RowDataPacket[]>(
      "SELECT upc.id, upc.category_id, upc.user_id, c.name " +
      "FROM user_prefer_category upc JOIN category c on upc.category_id = c.id " +
      "WHERE upc.user_id = ? ORDER BY upc.category_id ASC;",
      [userId]
    );
